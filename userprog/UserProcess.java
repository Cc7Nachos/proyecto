...
public class UserProcess {
    ...
    public UserProcess() {
        ...
        listFile = new LinkedList<OpenFile>();
        listFile.add(getNextPosition(),UserKernel.console.openForReading());
        listFile.add(getNextPosition(),UserKernel.console.openForWriting());
   ...
   /* private int handleJoin(int pid, int statusPtr) {
    ...
    public int handleRead(int posicion, int buffer, int count){
        OpenFile file = listFile.get(posicion);
        if(file != null){
    ...
    public int handleWrite(int posicion, int buffer, int con){
        OpenFile file = listFile.get(posicion);
        if(file != null){
            byte[] stream = new byte[pageSize];
            int of = 0;
            while(con > 0){
                int length = con;
                if(con>pageSize){ 
                ...
                int readedBytes = readVirtualMemory(buffer, stream, of, length);
                if(con > 0 && readedBytes == 0){ 
                    break;
                }
                int writedBytes = file.write(stream, 0, readedBytes);
                if(readedBytes != writedBytes){
                    return -1;
                } 
                con -= readedBytes;
    ...
    public int handleClose(int posicion){
        OpenFile file = listFile.get(posicion);
        if(file != null){
            file.close();
            listFile.remove(posicion);
            return 0;
        }
            return -1;
    }
    public int handleUnlink(int a0){
        String name = readVirtualMemoryString(a0,255);
        if(name != null){
    ...
    public int saveFile(OpenFile file){
        int x = getNextPosition();
        if(x != 1){
            listFile.add(x,file);
            return x;
        }
    ...
    public int getNextPosition(){
        for(int x = 0; x < 16; x++){
            OpenFile searching = listFile.get(x);
            if(searching == null){
                return x;
            }
        }
    ...
    /**Variables que utilizo (tabla)  */    
    private LinkedList<OpenFile> listFile;
